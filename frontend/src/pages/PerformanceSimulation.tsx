...
export default function PerformanceSimulation() {
  ...
  const sim = (artifacts as { performance_sim?: PerfData }).performance_sim;
  const kgraph = (artifacts as { keyword_graph?: { top_keywords?: { keyword: string; score: number }[]; clusters?: { id: number; keywords: string[] }[] } }).keyword_graph;
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-8 px-6 py-8">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Projected 30-day performance and keyword graph analysis.</p>
      </div>
      ...
      {kgraph && kgraph.top_keywords && kgraph.top_keywords.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <h2 className="font-display text-lg font-semibold text-slate-900">Keyword Graph (PageRank)</h2>
          <p className="mt-1 text-sm text-slate-600">Deterministic — NetworkX co-occurrence analysis, no LLM.</p>
          <div className="mt-4 flex flex-wrap gap-2">
            {kgraph.top_keywords.map((kw) => (
              <span
                key={kw.keyword}
                className="rounded-full border border-indigo-200 bg-indigo-50 px-3 py-1.5 text-xs font-semibold text-indigo-700"
                style={{ fontSize: `${Math.max(10, Math.min(16, 10 + kw.score * 600))}px` }}
              >
                {kw.keyword}
              </span>
            ))}
          </div>
          {kgraph.clusters && kgraph.clusters.length > 0 && (
            <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
              {kgraph.clusters.map((cl) => (
                <div key={cl.id} className="rounded-xl border border-slate-100 bg-slate-50 p-3">
                  <p className="text-[10px] font-bold uppercase text-slate-400">Cluster {cl.id}</p>
                  <p className="mt-1 text-xs text-slate-600">{cl.keywords.slice(0, 6).join(", ")}</p>
                </div>
              ))}
            </div>
          )}
